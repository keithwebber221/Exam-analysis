...
# ══════════════════════════════════════════════════════════════
PALETTE = [
    ("1F9E9E", "E0F4F4", "0D6E6E"),
    ("7D8B99", "EDF0F3", "4A5568"),
    ("8E6BB0", "F0E6FA", "5B3A7E"),
    ("C0392B", "FDECEA", "922B21"),
...
def _make_class_sheet(wb, cls, q_cols, max_scores, paper_map,
    ...
    _sc(ws["B2"], val=f"{cls}班　項目分析　·　本班 vs 全級　／　本班 vs {other_label}合拼",
        fill=_cfill(dark_hex), font=_fnt(bold=True, size=14, color="FFFFFF"), aln=_CTR)
    ws.row_dimensions[3].height = 14
    ws.merge_cells(f"B3:{get_column_letter(1+NCOLS)}3")
    _sc(ws["B3"], val=f"本班人數：{n_cls}　　{other_label} 合拼人數：{n_other}　　全級人數：{n_all}",
    ...
    headers = [
        ...
        ("全　級",         "5D6A77", "EAECEE","3D4E5C",["平均分","平均%","平均%圖表","S.D."],       4),
    ...
    for title, title_bg, sub_bg, sub_fg, sub_labels, span in headers:
        ...
        _sc(ws.cell(4, col), val=title, fill=_cfill(title_bg),
            font=_fnt(bold=True, size=12, color="FFFFFF"), aln=_CTR)
        for o, lbl in enumerate(sub_labels):
            _sc(ws.cell(5, col+o), val=lbl,
                fill=_cfill(sub_bg), font=_fnt(bold=True, size=10, color=sub_fg), aln=_CTR)
        col += span
    ...
    for ri, q in enumerate(q_cols):
        ...
        if paper != prev_paper:
            ...
            _sc(ws.cell(data_row, 2),
                val=f"　{paper}　·　滿分 {int(p_max)}　·　差距圖表刻度 ±{int(round(scale*100))}%",
                fill=_cfill(dark_hex), font=_fnt(bold=True, size=11, color="FFFFFF"), aln=_LFT)
            data_row += 1
            prev_paper = paper
        ...
        _sc(ws.cell(data_row, dc), val=int(mx), fill=bg, font=_fnt(size=11),            aln=_CTR, bdr=THIN, fmt="0"); dc+=1
        _sc(ws.cell(data_row, dc), val=n_cls,          fill=bg, font=_fnt(size=11), aln=_CTR, bdr=THIN, fmt="0"); dc+=1
        _sc(ws.cell(data_row, dc), val=cs["mean"],     fill=bg, font=_fnt(size=11), aln=_CTR, bdr=THIN, fmt="0.00"); dc+=1
        _sc(ws.cell(data_row, dc), val=cs["mean_pct"], fill=bg, font=_fnt(size=11), aln=_CTR, bdr=THIN, fmt="0.0%"); dc+=1
        ...
        _sc(ws.cell(data_row, dc), val=cs["std"],      fill=bg, font=_fnt(size=11), aln=_CTR, bdr=THIN, fmt="0.00"); dc+=1

        gf = _cfill("F2F3F4")
        _sc(ws.cell(data_row, dc), val=gs["mean"],     fill=gf, font=_fnt(size=11,color="3D4E5C"), aln=_CTR, bdr=THIN, fmt="0.00"); dc+=1
        _sc(ws.cell(data_row, dc), val=gs["mean_pct"], fill=gf, font=_fnt(size=11,color="3D4E5C"), aln=_CTR, bdr=THIN, fmt="0.0%"); dc+=1
        ...
        c1 = ws.cell(data_row, dc); c1.value=bar1; c1.font=Font(name="Courier New",size=11,color=("1A6B3A" if d1>=0 else "9B1C1C")); c1.alignment=_CTR; c1.border=THIN; c1.fill=_diff_fill(d1); dc+=1

        of = _cfill("F5EEF8")
        _sc(ws.cell(data_row, dc), val=os_["mean"],     fill=of, font=_fnt(size=11,color="6C3483"), aln=_CTR, bdr=THIN, fmt="0.00"); dc+=1
        _sc(ws.cell(data_row, dc), val=os_["mean_pct"], fill=of, font=_fnt(size=11,color="6C3483"), aln=_CTR, bdr=THIN, fmt="0.0%"); dc+=1
        ...
        c2 = ws.cell(data_row, dc); c2.value=bar2; c2.font=Font(name="Courier New",size=11,color=("1A6B3A" if d2>=0 else "9B1C1C")); c2.alignment=_CTR; c2.border=THIN; c2.fill=_diff_fill(d2)
...
def _make_distribution_sheet(wb, df, max_scores, paper_map, class_col, classes, palette):
    ...
    _sc(ws["B2"], val="分數分佈　·　各卷及合併總分",
        fill=_cfill("1F3864"), font=_fnt(bold=True,size=16,color="FFFFFF"), aln=_CTR)
    ...
    for sec_label, raw_series, s_max in sections:
        ...
        _sc(ws.cell(cur_row,2),
            ...
            fill=_cfill("2C3E50"), font=_fnt(bold=True,size=12,color="FFFFFF"),
        ...
        for cls in classes:
            ...
            _sc(ws.cell(cur_row,col), val=cls, fill=_cfill(palette[cls][2]),
...
def _make_heatmap_sheet(wb, class_stats, grade_stats, classes, q_cols, paper_map, palette):
    ...
    for i, cls in enumerate(classes):
        _sc(ws.cell(3,3+i), val=cls, fill=_cfill(palette[cls][2]),
            font=_fnt(bold=True,size=11,color="FFFFFF"), aln=_CTR)
    ...
    for q in q_cols:
        ...
        if paper != prev_paper:
            ...
            ws.merge_cells(start_row=row, start_column=2, end_row=row, end_column=1+NCOLS)
            _sc(ws.cell(row,2), val=f"  {paper}", fill=_cfill("2C3E50"),
                font=_fnt(bold=True,size=11,color="FFFFFF"), aln=_LFT)
            row += 1; prev_paper = paper

        ws.row_dimensions[row].height = 17
        _sc(ws.cell(row,2), val=q, fill=_cfill("EBF5FB"),
            font=_fnt(bold=True,size=11,color="1A5276"), aln=_CTR)

        pcts = [class_stats[cls][q]["mean_pct"] for cls in classes] + [grade_stats[q]["mean_pct"]]
        mn, mx2 = min(pcts), max(pcts)
        for i, cls in enumerate(classes):
            pct = class_stats[cls][q]["mean_pct"]
            t = (pct - mn)/(mx2-mn) if mx2>mn else 0.5
            r2 = int(255-t*150); g2 = int(180+t*75); b2 = int(150-t*50)
            hex_bg = f"{r2:02X}{g2:02X}{b2:02X}"
            txt_color = "FFFFFF" if t<0.35 or t>0.85 else "1A1A1A"
            _sc(ws.cell(row,3+i), val=pct, fill=_cfill(hex_bg),
                font=_fnt(size=11,color=txt_color), aln=_CTR, fmt="0.0%")
        _sc(ws.cell(row,3+len(classes)), val=grade_stats[q]["mean_pct"],
            fill=_cfill("EAECEE"), font=_fnt(bold=True,size=11,color="3D4E5C"),
            aln=_CTR, fmt="0.0%")
        row += 1
